-- =====================================================================
-- Route pg_cron's process_scheduled_posts_by_time() webhook by token_type
-- =====================================================================
-- `handle_scheduled_post_webhook` (the row-level trigger) was already
-- updated in `20260418_scheduled_post_webhook_route_by_token_type.sql`
-- to pick the correct n8n webhook based on `clients.token_type`.
--
-- BUT `process_scheduled_posts_by_time` (called by pg_cron every minute
-- for non-immediate posts) still hardcodes the old webhook URL and does
-- not forward `token_type`. Clients connected via Instagram Business
-- Login would silently hit the wrong workflow.
--
-- This migration mirrors the routing logic in that function.
...
BEGIN
  ...
  FOR post_record IN
    SELECT
      ...
      c.token_type as client_token_type,
  ...
  LOOP
    BEGIN
      ...
      IF FOUND THEN
        -- Route based on the client's token_type.
        IF post_record.client_token_type = 'instagram_business' THEN
        ...
        webhook_payload := jsonb_build_object(
          ...
          'record', jsonb_build_object(
            ...
            'client_data', jsonb_build_object(
              ...
              'token_type',           COALESCE(post_record.client_token_type, 'facebook_login')
            )
          ),
        ...
        BEGIN
          ...
          RAISE NOTICE 'Post % processado e webhook enviado para % (request_id: %)',
            post_record.id, target_url, webhook_result;
