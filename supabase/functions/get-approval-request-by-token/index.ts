...
serve(async (req) => {
  ...
  try {
    ...
    const { data: postsRows, error: postsError } = await supabase
      ...
      .select('id, caption, images, video, cover_image, post_type, scheduled_date, approval_status, approval_feedback, approval_responded_at')
      .in('id', postIds)
    ...
    const orderMap = new Map(junctionRows.map((r: { scheduled_post_id: string; sort_order: number }) => [r.scheduled_post_id, r.sort_order]));
    const posts = postsRows
      .sort((a: { id: string }, b: { id: string }) => (orderMap.get(a.id) ?? 0) - (orderMap.get(b.id) ?? 0))
